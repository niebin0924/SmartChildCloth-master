...
@interface MyPosterViewController () <UITableViewDataSource,UITableViewDelegate,HJCActionSheetDelegate,popSheetDelegate>
{
    NSInteger currPage;
}
...
@implementation MyPosterViewController
...
- (void)request
{
    NSString *url = @"circle/findMyPost";
...
- (void)popSheetDelegate:(QuanZiTableViewCell *)cell
{
    ...
    if (self.selectCell.collected) {
        ...
        sheet = [[HJCActionSheet alloc]initWithDelegate:self CancelTitle:@"取消" OtherTitles:@"取消收藏",@"删除", nil];
    }else{
        sheet = [[HJCActionSheet alloc]initWithDelegate:self CancelTitle:@"取消" OtherTitles:@"收藏",@"删除", nil];
    }
