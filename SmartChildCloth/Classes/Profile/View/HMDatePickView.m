...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.backgroundColor = [UIColor colorWithWhite:0.227 alpha:0.3];
...
#pragma mark -- 选择器
- (void)configurationIsHasNavbar:(BOOL)hasNavbar {
    //时间选择器
    UIView *dateBgView;
    if (hasNavbar) {
        dateBgView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 200 - 64, [UIScreen mainScreen].bounds.size.width, 200)];
    }else{
        dateBgView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 200, [UIScreen mainScreen].bounds.size.width, 200)];
    }
